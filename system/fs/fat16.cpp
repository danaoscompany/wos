...
int fat16_get_time(int hour, int minute, int second) {
	return (hour<<11)|(minute<<5)|(second/2);
}

int fat16_format(char* path, char* diskLabel, int bytesPerSector, int sectorsPerCluster) {
	...
	int sectorsPerFat = availableClusters*2/bytesPerSector;
	//char* vbr = (char*)&fat16_vbr_template;
	char vbr[512];
	...
	//memcpy((char*)((int)vbr+82), "FAT16   ", 8);
	vbr[510] = 0x55;
	vbr[511] = 0xAA;
	...
	int fatSectors = availableClusters*2/bytesPerSector;
	// Empty FAT table
...
int fat16_delete_file(char* path) {
	...
	if (!fileFound) {
		...
		int fatPos = rootDirCluster*2/bytesPerSector;
		char fat[bytesPerSector];
		read_sector(storage, startingSector+reservedSectors+fatPos, 0, 1, fat);
		int fatEntryPos = rootDirCluster*2-(fatPos*bytesPerSector);
...
int fat16_update_file(char* path, char* content, int size) {
	...
	if (fileFound) {
		if (nextToken == totalToken) {
			...
			follow:;
			int fatEntryPos = clusterNumber*2;
...
File* fat16_open_file(char* path) {
	...
	if (fileFound) {
		...
		int fatPos = clusterNumber*2/bytesPerSector;
		char fat[bytesPerSector];
		read_sector(storage, startingSector+reservedSectors+fatPos, 0, 1, fat);
		int fatEntryPos = clusterNumber*2-(fatPos*bytesPerSector);
