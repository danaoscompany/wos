...
int fat32_get_time(int hour, int minute, int second) {
...
int fat32_format(char* path, char* diskLabel, int bytesPerSector, int sectorsPerCluster) {
	...
	//char* vbr = (char*)&fat32_vbr_template;
	char vbr[512];
	...
	int sectorsPerFat = availableClusters*4/bytesPerSector;
	vbr[36] = sectorsPerFat&0xFF;
	vbr[37] = sectorsPerFat>>8&0xFF;
	vbr[38] = sectorsPerFat>>16&0xFF;
	vbr[39] = sectorsPerFat>>24;
	vbr[44] = 0x02;
	vbr[45] = 0x00;
	vbr[46] = 0x00;
	vbr[47] = 0x00;
	//vbr[48] = 0x01; //Sector of FSInfo structure
	//vbr[49] = 0x00;
	//vbr[50] = 0x06; //Sector of backup boot sector
	//vbr[51] = 0x00;
	if (p->type == FLOPPY) {
		vbr[64] = 0x00;
	...
	/*vbr[67] = serialNumber&0xFF;
	vbr[68] = serialNumber>>8&0xFF;
	vbr[69] = serialNumber>>16&0xFF;
	vbr[70] = serialNumber>>24;*/
	...
	//memcpy((char*)((int)vbr+71), diskLabel, 11);
	//memcpy((char*)((int)vbr+82), "FAT32   ", 8);
	vbr[510] = 0x55;
	vbr[511] = 0xAA;
	...
	int prevFatSectors = ucharstouint(prev_vbr[39], prev_vbr[38], prev_vbr[37], prev_vbr[36]);
	...
	int prevDataStart = prevFatTableStart+(2*prevFatSectors);
	int prevRootDirSector = (2-2)*sectorsPerCluster+prevDataStart;
	// Empry previous root directory
	char* rootDir = (char*)malloc(sectorsPerCluster*bytesPerSector);
	memset(rootDir, 0, sectorsPerCluster*bytesPerSector);
	write_sector(storage, prevRootDirSector, 0, sectorsPerCluster, rootDir);
	free(rootDir);
	int fatTableStart = p->startingSector+RESERVED_SECTORS;
	int fatSectors = availableClusters*4/bytesPerSector;
	// Empty FAT table
	memset32u(fat, 0x0FFFFFF8, 1);
	memset32u((unsigned char*)((int)fat+4), 0xFFFFFFFF, 1);
	memset32u((unsigned char*)((int)fat+8), 0x0FFFFFFF, 1);
	...
	memset32(fat, 0x0FFFFFF8, 1);
	memset32((unsigned char*)((int)fat+4), 0xFFFFFFFF, 1);
	memset32((unsigned char*)((int)fat+8), 0x0FFFFFFF, 1);
	...
	rootDir = (char*)malloc(sectorsPerCluster*bytesPerSector);
	memset(rootDir, 0, sectorsPerCluster*bytesPerSector);
	int dataStart = p->startingSector+RESERVED_SECTORS+2*fatSectors;
	write_sector(storage, dataStart, 0, sectorsPerCluster, rootDir);
	free(rootDir);
	...
	write_sector(storage, 0x06, 0, 1, mbr);
	return 0;
}
...
int fat32_delete_file(char* path) {
	...
	int rootDirStart = (int)vbr[47]*0x1000000+(int)vbr[46]*0x10000+(int)vbr[45]*0x100+(int)vbr[44];
	int sectorsPerFat = ucharstouint(vbr[39], vbr[38], vbr[37], vbr[36]);
	...
	char* rootDir = (char*)malloc(sectorsPerCluster*bytesPerSector);
	int dataStart = startingSector+reservedSectors+fatCount*sectorsPerFat;
	int nextToken = 1;
	path = strcombine(path, "/");
	int totalToken = get_total_char_from_string(path, '/');
	int rootDirCluster = dataStart;
	read_file_again:;
	...
	read_sector(storage, rootDirCluster, 0, sectorsPerCluster, rootDir);
	while (rootDir[i] != 0) {
		fileEntryPos = i;
	...
	if (!fileFound) {
		...
		int fatPos = rootDirCluster*4/bytesPerSector;
		char fat[bytesPerSector];
		read_sector(storage, startingSector+reservedSectors+fatPos, 0, 1, fat);
		int fatEntryPos = rootDirCluster*4-(fatPos*bytesPerSector);
	...
	if (fileFound) {
		...
		if (nextToken == totalToken) {
			...
			for (int j=0; j<allFatSectors; j++) {
				...
				for (int k=0; k<512; k++) {
					int fatEntry = ucharstouint(fat[fatOffset+3], fat[fatOffset+2], fat[fatOffset+1], fat[fatOffset]);
			...
			if (makeDirectory) {
				...
				write_sector(storage, dataSector, 0, bytesPerCluster/sectorsPerCluster, directory);
				return 0;
			}
			...
			for (int j=0; j<neededClusters; j++) {
				...
				write_sector(storage, dataSector, 0, bytesPerCluster, content);
				// Update fat table
...
int fat32_update_file(char* path, char* content, int size) {
	...
	if (fileFound) {
		if (nextToken == totalToken) {
			...
			follow:;
			int fatEntryPos = clusterNumber*4;
			//printf("FAT entry position: %d\n", fatEntryPos);
			int fatPos = (fatEntryPos/512);
			char fat[512];
			read_sector(storage, startingSector+reservedSectors+fatPos, 0, 1, fat);
			int fatOffset = fatEntryPos-(fatPos*512);
			unsigned int fatEntry = ucharstouint(fat[fatOffset+3], fat[fatOffset+2], fat[fatOffset+1], fat[fatOffset]);
					...
					for (int j=0; j<allFatSectors; j++) {
						...
						for (int k=0; k<512; k++) {
							...
							if (fatEntry == 0x00000000) {
								...
								lastCluster = fatEntryPos/4;
								printf("Last cluster: %x\n", lastCluster);
								printf("Previous FAT offset: %d\n", prevFatOffset);
								printf("FAT offset: %d\n", fatOffset);
								tmpFat[fatOffset] = 0xFF;
								tmpFat[fatOffset+1] = 0xFF;
								tmpFat[fatOffset+2] = 0xFF;
								tmpFat[fatOffset+3] = 0x0F;
								tmpFat[prevFatOffset] = lastCluster&0xFF;
								tmpFat[prevFatOffset+1] = lastCluster>>8&0xFF;
								tmpFat[prevFatOffset+2] = lastCluster>>16&0xFF;
								tmpFat[prevFatOffset+3] = lastCluster>>24;
...
File* fat32_open_file(char* path) {
	...
	if (fileFound) {
		...
		int fatPos = clusterNumber*4/bytesPerSector;
		char fat[bytesPerSector];
		read_sector(storage, startingSector+reservedSectors+fatPos, 0, 1, fat);
		int fatEntryPos = clusterNumber*4-(fatPos*bytesPerSector);
		int fatEntry = ucharstouint(fat[fatEntryPos+3], fat[fatEntryPos+2], fat[fatEntryPos+1], fat[fatEntryPos]);
		if (fatEntry >= 0x0FFFFFF8) {
